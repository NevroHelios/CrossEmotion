        ...
        with torch.autocast(device_type=self.device.type, enabled=True):
            ...
            for epoch in range(num_epochs):
                ...
                for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
                    ...
                    running_acc += self.calculate_acc(labels, preds) * image_data.size(0)
...
if __name__ == "__main__":
    ...
    train_loader = ifeed_dataloader(data_dir="data/ifeed/IFEED_Base/Training", 
                                    batch_size=32, 
                                    shuffle=True)
    ...
    engine.train(optimizer=optimizer, loss_fn=loss_fn, train_loader=train_loader, num_epochs=10)