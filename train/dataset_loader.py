...
class MeldDataset(Dataset):
    ...
    def _read_video_frames(self, video_path: str):
        ...
        try:
            while (
                len(frames) < 60
            ):  # read 60 frames -> approx 2 secs at 30fps (might increase if needed)
                ret, frame = cap.read()
                if not ret:
                    break
    ...
    def _extract_audio_features(self, video_path: str):
        wave, sr = torchaudio.load(video_path)
        if sr != 16000:
    ...
    def __getitem__(self, index: int | torch.Tensor) -> Any:
        try:
...
if __name__ == "__main__":
    ...
    for batch in train_loader:
        print(batch["text_inputs"]["input_ids"].shape)  # (batch_size, 128)
        print(batch["audio_features"].shape)  # (batch_size, 64, 300)
        print(batch["video_frames"].shape)  # (batch_size, 60, 3, H, W)
        print(batch["emotion"])  # (batch_size,)
        print(batch["sentiment"])  # (batch_size,)
        break
